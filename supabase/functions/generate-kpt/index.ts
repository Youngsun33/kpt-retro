...
10. 입력이 짧더라도 keep/problem/try 각각 최소 2개 이상 생성해
11. 하나의 문장(마침표 기준)은 반드시 하나의 항목으로 분리해
12. 여러 의미가 포함된 문장은 반드시 분해해서 각각 다른 항목으로 나눠
13. 마침표(.), 줄바꿈, 그리고 '그리고', '해서', '하면서' 등으로 이어진 문장은 각각 독립된 항목으로 분리해
14. 하나의 항목에는 반드시 하나의 사건/행동만 포함해야 해
15. 두 개 이상의 행동이 들어가면 반드시 쪼개서 여러 배열 요소로 만들어
